...
<head>
    ...
    <title>Calendar - ProjectDesk</title>
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4" style="margin-top: 60px;">
            <div class="calendar-wrapper">
                <div class="calendar-nav text-center">
                    <button class="btn btn-lg calendar-type-btn active" id="taskView">
                        <i class="fa fa-tasks me-2"></i> Tasks Calendar
                    </button>
                    <button class="btn btn-lg calendar-type-btn" id="noteView">
...
.note-event {
    background: rgb(2, 170, 83) !important;  /* Solid green background */
    color: #ffffff !important;  /* White text */
    border: none !important;
    font-weight: 600 !important;
    border-radius: 4px !important;
    padding: 8px 12px !important;
    margin: 3px 4px !important;
    box-shadow: 0 2px 4px rgba(2, 170, 83, 0.2) !important;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: function(info, successCallback, failureCallback) {
            fetch('get_calendar_events.php?view=' + currentView)
        ...
        eventClick: function(info) {
            ...
            modalTitle.innerHTML = `
                <i class="fa ${event.extendedProps.type === 'task' ? 'fa-tasks' : 'fa-sticky-note'} me-2"></i>
                ${event.title}`;
            ...
            eventType.className = 'badge rounded-pill ' + 
                (event.extendedProps.type === 'task' ? 'bg-primary' : 'bg-success');
            eventType.textContent = event.extendedProps.type === 'task' ? 'Task' : 'Note';
            ...
            if (event.extendedProps.type === 'task' && event.extendedProps.status) {
            ...
            // Set correct view details link based on type and role
    ...
    // Switch between task and note views
    document.getElementById('taskView').addEventListener('click', function() {
        currentView = 'task';
        updateActiveButton(this);
        calendar.refetchEvents();
    });

    document.getElementById('noteView').addEventListener('click', function() {
        currentView = 'note';
        updateActiveButton(this);
        calendar.refetchEvents();
    });
